...
export const scenes = {
    huimen_exp_dog_bark: createScene('huimen_exp_dog_bark', {title:'犬吠',text:`你经过一户人家，听见里面有狗叫。
...
[red]它跑的方向，是秀兰的坟。[/red]`,effects:{"sanity":-3,"yin":1},choices:[createChoice({"text":"跟着黑狗","next":"huimen_exp_follow_dog","effects":{"yin":1}}),createChoice({"text":"不管它，回村街","next":"huimen_exp_village_street","effects":{"yin":1}})]}),
    huimen_exp_follow_dog: createScene('huimen_exp_follow_dog', {title:'引路',text:`你跟着黑狗跑。
...
你蹲下身，发现它已经变成了一尊石狗。`,effects:{"sanity":-5,"yin":2,"setFlag":"huimen_exp_knows_bone_locations"},choices:[createChoice({"text":"回村街","next":"huimen_exp_village_street","effects":{"yin":1}})]}),
    huimen_exp_fox_fire: createScene('huimen_exp_fox_fire', {title:'狐火',text:`村外的坟地里飘起几团绿色的火焰。
...
可你也听说过，跟狐火走的人，多半回不来。`,effects:{"sanity":-5,"yin":2},choices:[createChoice({"text":"跟着狐火","next":"huimen_exp_follow_fox_fire","effects":{"yin":2}}),createChoice({"text":"不跟，回村街","next":"huimen_exp_village_street","effects":{"sanity":3,"yin":-1}})]}),
    huimen_exp_follow_fox_fire: createScene('huimen_exp_follow_fox_fire', {title:'狐径',text:`你跟着狐火走进树林。
...
"这是秀兰的怨念烧起来的样子。" 狐狸说，"你出去，村子就烧。你留下，村子就还在。"`,effects:{"sanity":-8,"yin":3},choices:[createChoice({"text":"出去，让村子烧","next":"huimen_exp_fox_fire_escape","effects":{"sanity":-5,"yin":5}}),createChoice({"text":"留下，找别的办法","next":"huimen_exp_village_street","effects":{"sanity":5,"yin":-3}})]}),
    huimen_exp_fox_fire_escape: createScene('huimen_exp_fox_fire_escape', {title:'狐火引路',text:`你踏进了铜镜。
...
可你再也没有梦见过秀兰。你只梦见火，梦见纸人在火里跳舞，梦见它们的脸都是你的脸。`,effects:{"sanity":-10,"yin":10},ending:'huimen_exp_moonlit_escape'})
};
