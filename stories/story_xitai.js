/**
 * 《戏台》—— 阴阳簿 · 第三卷
 *
 * 世界观：荒废二十年的老戏台突然又唱起了戏。
 * 村里老人说，那是给鬼听的。戏不散，人莫近。
 */

export const StoryData = {
    prologue: {
        title: '青石戏台',
        text: `青石镇往东三里，有一座老戏台。

二十年前，春香班在这里演出《牡丹亭》，戏台突然坍塌，烧死了十七个人。从那以后，镇上的人把这里封了，说是"鬼台"。

你是省民俗研究所的研究员，专门记录民间传说。你听说了青石戏台的事，带着录音设备和一台老式相机，在一个阴雨绵绵的下午来到了这里。

戏台比想象中保存得更完好。

朱红的柱子只是褪了色，戏台上的幕布虽然破烂，却还挂着。最诡异的是，台中央摆着一套完整的锣鼓，锣面上干干净净，像是有人刚擦过。

你举起相机，按下快门。

[red]取景框里，戏台上站满了穿戏服的人。[/red]

你猛地放下相机。戏台上空无一人。

再看取景框，那些人还在。他们背对着你，正在化妆。`,
        effects: {
            sanity: -10,
            yin: 10
        },
        choices: [
            {
                text: '拍照记录',
                next: 'take_photo'
            },
            {
                text: '爬上戏台查看',
                next: 'climb_stage'
            },
            {
                text: '先找村里老人打听',
                next: 'ask_villagers'
            },
            {
                text: '你阴气太重，看见后台真正的戏班正在上妆',
                next: 'true_backstage',
                condition: { yinAbove: 15 },
                hidden: true,
                effects: { sanity: -5, yin: 10 }
            }
        ]
    },

    // 真正的后台（新增隐藏分支）
    true_backstage: {
        title: '真正的后台',
        text: `你放下相机，揉了揉眼睛。

戏台并没有空。幕布后面，十几个穿戏服的人正在忙碌。有人对着镜子画脸，有人整理水袖，有人调弦试嗓。

他们不是幻象。他们是真实的——至少对你来说，此刻是真实的。

你看见一个穿红戏服的女子坐在最里面的位置，由两个丫鬟模样的人伺候着上妆。她的脸还没画完，但眉眼已经能看出惊人的美。

"云袖姑娘，" 一个老妇人模样的人说，"今天唱《惊梦》，您可别再像上次那样，唱到一半就哭了。"

云袖笑了笑："最后一次了。唱完这出，我就跟他走。"

她说的"他"，是谁？

你正想走近，一阵冷风吹过，后台的景象像水波一样晃了晃，消失了。戏台又恢复了破败荒凉的样子。

但你心里清楚，你刚才看见的，是二十年前火灾发生前的最后一刻。`,
        effects: {
            sanity: -10,
            yin: 15,
            setFlag: 'sawTrueBackstage'
        },
        choices: [
            {
                text: '去后台查看',
                next: 'backstage'
            },
            {
                text: '找镇上老人打听',
                next: 'ask_villagers'
            }
        ]
    },

    take_photo: {
        title: '鬼影',
        title: '鬼影',
        text: `你又按了几下快门。

每张照片里，戏台上的人都比前一张更多。他们从后台走出来，排队上妆，像是准备开演。

最后一张照片里，他们齐刷刷地转过头，看向镜头。

[red]每一张脸都没有五官，只有一片空白。[/red]

你的相机突然罢工了。胶卷仓自动弹开，里面的胶卷已经变成了焦黑色，像是被火烧过。

一阵冷风吹过，戏台上的幕布轻轻飘动。

你听见后台传来一声清嗓：

[whisper]"开——戏——"[/whisper]`,
        effects: {
            sanity: -20,
            yin: 15,
            addItem: '烧焦的胶卷'
        },
        choices: [
            {
                text: '冲进后台',
                next: 'backstage'
            },
            {
                text: '转身离开戏台',
                next: 'try_leave'
            }
        ]
    },

    climb_stage: {
        title: '登台',
        text: `你踩上戏台的木板。

木板发出"吱呀"一声，但承受住了你的重量。你走到台中央，脚下的位置有一块颜色更深的木板。

你蹲下来，发现那是一块被烧过的痕迹。痕迹的形状像是一个人形。

[red]这里就是当年有人被烧死的位置。[/red]

你伸手摸了摸那块木板。触感温热，像是刚刚有人躺在这里。

突然，锣鼓自己响了一声：

[scream]"锵——"[/scream]

你吓得一屁股坐在地上。`,
        effects: {
            sanity: -15,
            yin: 20,
            visual: 'shake'
        },
        choices: [
            {
                text: '查看烧痕周围的木板',
                next: 'inspect_burn_mark'
            },
            {
                text: '马上跳下戏台',
                next: 'jump_off_stage'
            }
        ]
    },

    ask_villagers: {
        text: `你回到镇上，找到一家小卖部。

店老板是个六十多岁的老头，听说你要去老戏台，脸色当时就变了。

"那地方去不得。" 老头压低声音，"二十年前那场火，不是意外。"

"不是意外？" 你追问。

老头左右看看，凑近你："春香班的班主得罪了人。演出那天，有人在戏台底下倒了桐油。火是从台下烧起来的，台上的人一个都没跑掉。"

"谁干的？"

老头摇摇头："不知道。但从那以后，每年七月半，老戏台都会自己唱戏。唱的还是那场没唱完的《牡丹亭》。"

他递给你一支烟："你要去也行，记住一句话——戏不散，人莫近。要是听见戏停了，马上跑。"

你接过烟，目光落在柜台后一堆没卖完的纸扎上。那些纸人纸马糊得精致，每个纸人胸口都贴着一张小小的黄签，签上印着"周氏纸扎铺"几个字。你问老头这铺子在哪里，老头摆摆手："青石镇东头，早没人了。"`,
        effects: {
            sanity: -5,
            setFlag: 'knowsArson'
        },
        choices: [
            {
                text: '问谁可能知道真相',
                next: 'ask_suspects'
            },
            {
                text: '返回戏台',
                next: 'return_to_stage'
            }
        ]
    },

    backstage: {
        title: '后台',
        text: `你掀开后台的布帘。

后台很小，摆着几面破旧的镜子和一张化妆台。化妆台上放着一个打开的脸谱盒，里面的油彩还是湿的。你顺手拉开抽屉，里面散落着几张戏票和一把断了齿的木梳。最底下压着一张泛黄的纸条，上面用工整的小楷写着几句词：

[faded]"来呀，水凉快呀……
水下没有孤灯，水下没有长夜……"[/faded]

你不认得这曲调，但纸条边缘沾着水渍，像是被人从河边带回来的。

墙上贴着一张戏单：

[faded]"今日剧目：《牡丹亭》
杜丽娘 — 云袖
柳梦梅 — 周生
花神 — 众伶
司鼓 — 老赵"[/faded]

你注意到，"柳梦梅"的名字被人用指甲划掉了，旁边写了一个血红的"死"字。

镜子突然亮了一下。你看见镜子里，一个穿小生戏服的男人站在你身后，正把脸贴在你肩上。

[red]他没有脸。[/red]`,
        effects: {
            sanity: -25,
            yin: 20,
            visual: 'flicker'
        },
        choices: [
            {
                text: '问他是谁',
                next: 'ask_ghost_actor'
            },
            {
                text: '用相机拍镜子',
                next: 'mirror_photo'
            },
            {
                text: '头也不回跑出后台',
                next: 'run_from_backstage'
            }
        ]
    },

    // 镜中留影（新增场景，补全引用）
    mirror_photo: {
        title: '镜中留影',
        text: `你举起相机，对准那面镜子。

闪光灯亮起的瞬间，镜子里的小生戏服男人猛地抬起头。他没有五官的脸正对着镜头，像是在笑。

[red]咔嚓。[/red]

相机里吐出一张照片。你低头一看，照片上不是镜子，而是一张戏台。

戏台上，一个穿红戏服的女子正在唱《惊梦》。她身边站着一个穿小生戏服的男人，但男人的脸被划掉了，只剩一个血红的"死"字。

照片背面，有人用钢笔写着一行小字：

[faded]"民国二十三年，青石镇大戏台。云袖殁，周生殉。戏不散，魂不归。"[/faded]

你忽然明白，这戏台不是荒废了二十年。它是被封印了二十年。`,
        effects: {
            sanity: -15,
            yin: 10,
            setFlag: 'knowsStageTruth'
        },
        choices: [
            {
                text: '把照片收好，继续调查',
                next: 'ask_villagers'
            },
            {
                text: '烧了这张照片',
                next: 'burn_photo',
                effects: { sanity: 5, yin: -5 }
            }
        ]
    },

    // 烧照片（新增场景）
    burn_photo: {
        title: '焚照',
        text: `你用打火机点燃照片。

火苗舔上照片边缘，戏台上的云袖忽然转过头，像是隔着时空看了你一眼。

照片烧成灰烬，风里传来一声若有似无的叹息。

[whisper]"你和他们一样……都想让我消失。"[/whisper]

你手中的灰烬被风吹散，落在化妆台的镜子上，拼出两个字：

[red]"别走。"[/red]`,
        effects: {
            sanity: -10,
            yin: 15,
            setFlag: 'burned_stage_photo'
        },
        choices: [
            {
                text: '冲出后台',
                next: 'run_from_backstage'
            }
        ]
    },

    try_leave: {
        text: `你转身往戏台外走。

但来时的路不见了。戏台四周本来是一片荒地，现在却出现了一圈圈红色的灯笼，灯笼上写着"春香班"三个字。

你沿着灯笼走，走了很久，却发现自己又回到了戏台前。

幕布已经拉开了。

戏台上站着一排穿戏服的人，他们没有五官，却齐齐朝你鞠躬：

[whisper]"恭请观众入座。"[/whisper]`,
        effects: {
            sanity: -20,
            yin: 15
        },
        choices: [
            {
                text: '找地方坐下看戏',
                next: 'watch_ghost_opera'
            },
            {
                text: '拒绝入座',
                next: 'refuse_seat'
            }
        ]
    },

    inspect_burn_mark: {
        text: `你仔细查看那块烧焦的人形痕迹。

痕迹的边缘很整齐，不像是火烧出来的，更像是有人被固定在这里烧。痕迹的头部位置，嵌着一枚发黑的银钗。

你捡起银钗。钗身上刻着两个小字：

[faded]"云袖"[/faded]

这就是当年云袖被烧死的位置。她不是被坍塌的戏台砸死的，她是被人绑在这里烧死的。

[red]这是一场谋杀。[/red]

远处传来锣鼓声，戏台上的幕布无风自动。`,
        effects: {
            sanity: -15,
            yin: 10,
            addItem: '云袖的银钗'
        },
        choices: [
            {
                text: '把银钗放回原处',
                next: 'return_hairpin',
                effects: { yin: -5 }
            },
            {
                text: '带走银钗作为证据',
                next: 'take_hairpin',
                effects: { yin: 10 }
            }
        ]
    },

    jump_off_stage: {
        text: `你跳下戏台，落地时脚踝一阵剧痛。

但你顾不上这些，拼命往镇子的方向跑。身后的锣鼓声越来越响，像是有整个戏班子在追你。

你跑出大约一里地，回头一看。

戏台还是静静地立在那里，幕布垂着，什么都没有。

你长舒一口气，以为自己逃掉了。

可当你转回头时，你面前站着一排穿戏服的人。他们没有五官，却齐齐朝你伸手：

[whisper]"观众还没散场，怎能离席？"[/whisper]`,
        effects: {
            sanity: -25,
            yin: 20,
            visual: 'shake'
        },
        choices: [
            {
                text: '跟他们回去',
                next: 'watch_ghost_opera'
            },
            {
                text: '继续逃跑',
                next: 'keep_running',
                ending: 'lostInFog'
            }
        ]
    },

    ask_suspects: {
        text: `老头想了想："当年和春香班有过节的，只有两个人。一个是镇上开绸缎庄的刘掌柜，他女儿想进春香班当学徒，被班主拒了。另一个是……"

老头压低声音："云袖的未婚夫，周生。"

"周生？"

"对。云袖唱红了，周生怕她不要他了，几次三番闹着要她退班。演出前一天，还有人看见他们在戏台后面吵架。"

你想起戏单上被划掉的"柳梦梅 — 周生"。

那个角色，是不是 cloud sleeve 亲手划掉的？`,
        effects: {
            sanity: -5,
            setFlag: 'knowsSuspects'
        },
        choices: [
            {
                text: '去查刘掌柜后人',
                next: 'liu_descendants'
            },
            {
                text: '去查周生后人',
                next: 'zhou_descendants'
            }
        ]
    },

    return_to_stage: {
        title: '重返戏台',
        text: `你回到老戏台时，天已经黑了。

戏台上点满了红色的灯笼，把整个台子照得如同白昼。锣鼓声、二胡声、唱腔声混杂在一起，热闹非凡。

但台下没有观众。

台上，一群没有五官的戏子正在演出《牡丹亭》。他们唱得很投入，身段优美，水袖翻飞。

你注意到，演杜丽娘的那个戏子，腰间系着一条红绳。

红绳的另一端，系在台柱上。那绳结打得古怪，不是戏班子里常见的系法，倒像是你曾在某座深山村落的冥婚喜房里见过的——把人和桩子绑成一对，象征"永不分离"。`,
        effects: {
            sanity: -20,
            yin: 20
        },
        choices: [
            {
                text: '入座看戏',
                next: 'watch_ghost_opera'
            },
            {
                text: '绕到戏台侧面',
                next: 'side_of_stage'
            },
            {
                text: '你摸到胸口的护身符，决定用它来抵抗鬼戏的魅惑',
                next: 'talisman_resist',
                condition: { hasItem: '护身符' },
                hidden: true,
                effects: { sanity: 5, yin: -10 }
            }
        ]
    },

    // 符镇戏台（新增分支）
    talisman_resist: {
        title: '符镇戏台',
        text: `你掏出护身符，贴在戏台的一根柱子上。

黄符纸触到朱红柱子的瞬间，戏台上的锣鼓声猛地停了。那些没有五官的戏子像是被按了暂停键，保持着千奇百怪的姿势，僵在原地。

"散了吧。" 你对着戏台说，"你们的戏，早就唱完了。"

戏子们的身影开始变淡，像被水晕开的墨。最后，台上只剩下一件红色的戏服，孤零零地挂在台柱上。

你走过去，发现戏服的内衬里缝着一张纸条：

[faded]"云袖绝笔：愿来世，再唱一出完整的《牡丹亭》。"[/faded]

你把戏服小心地叠好，带出戏台。

天快亮了。`,
        effects: {
            sanity: 10,
            yin: -20,
            addItem: '云袖的戏服',
            setFlag: 'talismanResistedOpera'
        },
        choices: [
            {
                text: '把戏服和银钗一起安葬',
                next: 'bury_yunxiu_costume',
                ending: 'hidden_songEnds'
            },
            {
                text: '把戏服带回研究所',
                next: '',
                ending: 'peacefulDeparture'
            }
        ]
    },

    ask_ghost_actor: {
        title: '无脸小生',
        text: `你举起相机对准镜子。

闪光灯亮起的瞬间，镜子里的小生发出一声惨叫。他的脸像玻璃一样碎裂，碎片落在地上，变成一滩黑水。

但黑水里浮现出一段画面：

[faded]年轻的周生跪在戏台底下，手里拿着一个油壶。云袖站在台上，正看着他。她没有喊，没有逃，只是静静地看着他。[/faded]

画面最后，云袖的嘴唇动了动。从口型看，她说的是：

[whisper]"我不怪你。"[/whisper]

黑水慢慢渗入地板，消失不见。`,
        effects: {
            sanity: -10,
            yin: 5,
            setFlag: 'knowsTruth'
        },
        choices: [
            {
                text: '去台前找云袖',
                next: 'find_yunxiu'
            },
            {
                text: '你想起秀兰要的不过是一句道歉，云袖也许也一样',
                next: 'truth_for_yunxiu',
                condition: { flag: 'knowsTruth' }
            },
            {
                text: '我替她唱完',
                next: 'agree_to_perform'
            },
            {
                text: '你害死了她，还想让我替你赎罪？',
                next: 'condemn_zhou'
            }
        ]
    },

    run_from_backstage: {
        text: `你头也不回地冲出后台。

但后台的门通向的不是戏台外，而是戏台上。

你站在台中央，面对着空荡荡的观众席。红色的灯笼一盏盏亮起，照亮了台下的"观众"。

[red]那些观众没有脸。[/red]

他们整齐地坐着，像是在等待开场。

锣鼓声响起，你被推搡着换上了一套小生戏服。戏服上有股焦糊味，像是刚从火里抢出来的。`,
        effects: {
            sanity: -30,
            yin: 25
        },
        choices: [
            {
                text: '配合演出',
                next: 'perform_opera'
            },
            {
                text: '挣脱戏服逃跑',
                next: 'tear_costume',
                effects: { sanity: -10 }
            }
        ]
    },

    watch_ghost_opera: {
        title: '鬼戏',
        text: `你找了张长凳坐下。

戏唱到一半，突然停了。

所有戏子齐刷刷地看向你。

"柳梦梅呢？" 一个声音问，"杜丽娘等不到柳梦梅，戏怎么散场？"

你意识到，他们需要你上台。

[red]他们需要有人演完柳梦梅。[/red]`,
        effects: {
            sanity: -20,
            yin: 20
        },
        choices: [
            {
                text: '上台扮演柳梦梅',
                next: 'perform_opera'
            },
            {
                text: '继续沉默',
                next: 'silence_punish',
                ending: 'trappedAudience'
            },
            {
                text: '你曾见过另一个被辜负的新娘，也许云袖需要的不是对拜',
                next: 'xiulan_wisdom',
                condition: { flag: 'xiulanMercy' }
            },
            {
                text: '你听过类似的低语，那旋律里也有戏台的余音',
                next: 'paper_whisper_melody',
                condition: { flag: 'heard_paper_whisper' },
                hidden: true,
                effects: { sanity: -5, yin: 5 }
            }
        ]
    },

    // 纸人余音（新增分支）
    paper_whisper_melody: {
        title: '纸人余音',
        text: `戏台上的唱腔让你愣了一下。

那不是普通的《牡丹亭》。在婉转的曲调里，你听见了一段更细、更凉的旋律——像是在某个山村老槐树下，从纸扎人身体里透出来的低语。

[whisper]"井……下……有……人……"[/whisper]

两段声音重叠在一起，像是一根细细的红绳，把云袖和另一个被活埋的新娘连在了一起。

你猛然意识到：云袖不是孤立的存在。她的死、她的怨、她的戏台，都和某个叫"周"的家族有关。

台上的戏子似乎也感觉到了什么，齐齐停下动作。云袖缓缓转向你，没有五官的脸上似乎有什么在流动。

"你也听过？" 她的声音很轻，"那你也……见过她？"

"见过。" 你说，"她叫秀兰。她等了三十年的名分。"

云袖沉默了很久。最后她说："我等的，不是名分。是一句对不起。"`,
        effects: {
            sanity: -10,
            yin: 10,
            setFlag: 'yunxiuLinkedToPaper'
        },
        choices: [
            {
                text: '去后台找周生',
                next: 'truth_for_yunxiu',
                condition: { flag: 'knowsTruth' }
            },
            {
                text: '上台替她唱完',
                next: 'perform_opera'
            }
        ]
    },

    refuse_seat: {
        text: `"我不看戏。" 你说。

台上的戏子们停下了动作。整个戏台陷入死寂。

然后，他们开始向你走来。步伐很慢，但每一步都带着水汽——他们的戏服在滴水。

"不看戏的人，" 领头的戏子说，"就永远留在戏里。"`,
        effects: {
            sanity: -30,
            yin: 25
        },
        choices: [
            {
                text: '转身狂奔',
                next: 'keep_running',
                ending: 'lostInFog'
            },
            {
                text: '跪下求饶',
                next: 'beg_for_exit',
                ending: 'trappedAudience'
            }
        ]
    },

    return_hairpin: {
        text: `你把银钗轻轻放回烧焦的痕迹上。

银钗接触木板的瞬间，发出一声轻微的嗡鸣。戏台上的锣鼓声停了。

你听见一个女子的声音在唱：

[whisper]"情不知所起，一往而深……"[/whisper]

唱腔渐渐远去，像是有人从戏台上走下来，又走了很远。

你再抬头时，戏台上的红灯笼全灭了。幕布垂着，仿佛什么都没发生过。

你平安走出了青石镇。`,
        effects: {
            sanity: 10,
            yin: -30
        },
        ending: 'peacefulDeparture'
    },

    take_hairpin: {
        text: `你把银钗装进口袋，准备带回去研究。

刚走出戏台，天就黑了。不是正常的黑，是像有人把墨汁泼在了天上。

戏台上的锣鼓声大作，这次不是一出戏，而是十几出戏同时开唱。无数戏子的声音混在一起，震得你头疼欲裂。

[red]你把她的命根子带走了，她不会让你走的。[/red]

你被无形的力量拖回了戏台。戏服一件一件套在你身上，油彩一层一层画在你脸上。

最后，你看见镜子里的自己。

你变成了云袖。`,
        effects: {
            sanity: -40,
            yin: 30
        },
        ending: 'becomeYunxiu'
    },

    keep_running: {
        text: `你拼命跑。

雾气越来越浓，你分不清方向。戏台上的锣鼓声忽远忽近，像是有无数戏班子在雾中跟着你。

你跑啊跑，跑到再也跑不动。

最后，你撞上了一根木桩。抬头一看，是戏台的台柱。

你一直在戏台周围打转。

台上，戏子们齐声唱道：

[whisper]"戏不散，人莫近。人既近，戏不散。"[/whisper]`,
        ending: 'lostInFog'
    },

    side_of_stage: {
        text: `你绕到戏台侧面，发现台基下有一个狭小的空间。

那是当年放火的人躲藏的地方。地上还放着一个生锈的油壶，壶身上有"刘记绸缎"四个字。

[red]放火的人不是周生，是刘掌柜。[/red]

你正想进一步查看，身后传来一个苍老的声音：

"年轻人，知道太多，不好。"

你回头，看见一个穿着绸缎马褂的老人站在那里。他的身体半透明，脚不沾地。`,
        effects: {
            sanity: -15,
            yin: 15,
            setFlag: 'knowsTrueKiller'
        },
        choices: [
            {
                text: '质问他为何放火',
                next: 'confront_liu'
            },
            {
                text: '悄悄后退',
                next: 'retreat_slowly'
            }
        ]
    },

    liu_descendants: {
        text: `你找到刘掌柜的后人，一个开杂货铺的中年男人。

听说你问老戏台的事，他脸色煞白："我爷爷……他晚年总说胡话，说什么对不起春香班。他死前留下一封信，说不许后人打开。"

你买了他店里最贵的烟，他终于把信拿了出来。

信上只有一句话：

[faded]"云袖不死，我女儿永无出头之日。"[/faded]

原来刘掌柜的女儿也想当旦角，但云袖压得她出不了头。刘掌柜为了女儿的前途，在戏台底下倒了桐油。

但火势失控，烧死了整个戏班。`,
        effects: {
            sanity: -10,
            setFlag: 'knowsTrueKiller'
        },
        choices: [
            {
                text: '回戏台公布真相',
                next: 'return_to_stage'
            }
        ]
    },

    zhou_descendants: {
        text: `你找到周生的后人，一个退休教师。

老人听说你问周生，沉默了很久："我爷爷那辈子，只做错了一件事。"

"什么事？"

"他没能救出云袖。" 老人说，"火起来的时候，他冲进戏台，但横梁砸下来，把他挡在外面。他看着云袖被烧死，从那以后就疯了。"

"那戏单上被划掉的名字……"

"那是他自己划的。" 老人说，"他说自己不配演柳梦梅。"

你想起镜子里那个没有脸的小生。原来他一直困在后台，不是在害人，是在赎罪。`,
        effects: {
            sanity: -5,
            setFlag: 'knowsZhouInnocent'
        },
        choices: [
            {
                text: '回戏台告诉云袖真相',
                next: 'return_to_stage'
            }
        ]
    },

    agree_to_perform: {
        text: `你点点头："我替她唱完。"

周生的身影从镜子里走出来，他的脸渐渐有了轮廓——是一个清秀的年轻人，满脸泪痕。

"谢谢。" 他说，"云袖等这句话，等了三十年。"

他把一件小生戏服递给你。戏服上有焦痕，但还能穿。

"记住，" 他说，"唱到'原来姹紫嫣红开遍'时，不要看她的眼睛。一看，你就再也走不了了。"`,
        effects: {
            sanity: -10,
            yin: 10,
            addItem: '小生戏服'
        },
        choices: [
            {
                text: '登台演出',
                next: 'perform_opera'
            }
        ]
    },

    condemn_zhou: {
        text: `你怒道："你害死了她，现在还想让我替你赎罪？"

周生的脸再次扭曲。他的声音变成了咆哮："你以为我想这样吗？！我每天都看着她再死一次！我每天都听见她在唱！"

戏台上的锣鼓声突然变得急促。周生的身影膨胀，化成一团黑雾向你扑来。

"既然你不愿意唱，" 他的声音从黑雾中传出，"那就永远听我们唱！"`,
        effects: {
            sanity: -30,
            yin: 25
        },
        ending: 'trappedAudience'
    },

    find_yunxiu: {
        text: `你走到台前。

演杜丽娘的戏子正站在台中央，腰间系着红绳。她没有五官，但你知道她就是云袖。

"我知道真相了。" 你说，"放火的是刘掌柜，不是周生。周生一直想救你。"

云袖的身体颤抖了一下。红绳无声地断裂。

"周生……" 她的声音很轻，"他在哪里？"

"他在后台。他不敢见你。"

云袖缓缓走向后台。你跟在她身后。

后台的镜子里，周生跪在地上。云袖走到镜前，伸出手，穿过了镜面。

他们的手握在一起。

[red]然后，一起消失了。[/red]

戏台上的红灯笼，一盏接一盏熄灭。`,
        effects: {
            sanity: 10,
            yin: -30
        },
        ending: 'reunion'
    },

    perform_opera: {
        title: '入戏',
        text: `你穿上小生戏服，走上戏台。

台下坐满了没有脸的观众。云袖站在你面前，水袖轻扬。

锣鼓响起，你开始唱：

[whisper]"则为你如花美眷，似水流年……"[/whisper]

你的声音不像自己的，像是周生在借你的口唱。

云袖越唱越近，越唱越近。你想起周生的警告，拼命不看她的眼睛。

但她在唱到"原来姹紫嫣红开遍"时，忽然摘下了自己的面具。

[red]面具下，是你的脸。[/red]

"你才是我的柳梦梅。" 她说，"从一开始就是。"`,
        effects: {
            sanity: -40,
            yin: 30
        },
        choices: [
            {
                text: '接受她',
                next: 'accept_yunxiu',
                ending: 'eternalDuet'
            },
            {
                text: '推开她，唱完最后一句',
                next: 'finish_opera',
                effects: { sanity: -10 }
            },
            {
                text: '你保持清醒，看穿这不过是幻术',
                next: 'see_through_illusion',
                condition: { sanityAbove: 80 },
                hidden: true,
                effects: { sanity: -5 }
            }
        ]
    },

    // 破幻（新增分支）
    see_through_illusion: {
        title: '破幻',
        text: `你深吸一口气，强迫自己不去看云袖的眼睛。

这不是真的。你是民俗研究所的研究员，你研究过无数类似的案例。戏台上的幻术，靠的就是让人相信自己就是戏中人。

"你不是在找柳梦梅。" 你说，"你在找一个人，替你记住云袖这个名字。"

云袖的动作停住了。

"你唱得很好。" 你继续说，"但戏已经散了三十年。周生死了，刘掌柜也死了。你还要困在这里多久？"

台下的"观众"开始骚动。他们的脸一张一张消失，像被擦掉的粉墨。

云袖的身影晃了晃。她脸上的面具——你的脸——出现了裂痕。

"我……不知道还能去哪里。" 她说。

"去该去的地方。" 你说，"我会记住你。不只是我，还有这块碑。"`,
        effects: {
            sanity: 10,
            yin: -20,
            setFlag: 'yunxiuIllusionBroken'
        },
        choices: [
            {
                text: '为她立一块碑',
                next: 'bury_yunxiu_memory',
                ending: 'hidden_songEnds'
            },
            {
                text: '唱完最后一句，送她走',
                next: 'finish_opera',
                effects: { sanity: -10 }
            }
        ]
    },

    tear_costume: {
        text: `你发疯似的撕扯戏服。

戏服像长在你身上一样，越撕越紧。台下没有脸的观众开始鼓掌，像是在看一场好戏。

"撕吧，" 云袖的声音说，"撕得越狠，你就越像柳梦梅。"

你低头一看，戏服已经和你的皮肤长在一起。红色的绸缎下，是你自己的血肉。

[red]你成了戏服的一部分。[/red]`,
        ending: 'becomeCostume'
    },

    silence_punish: {
        text: `你沉默着。

戏子们的脸开始融化，戏服开始燃烧。但他们还在唱，即使喉咙被火吞噬，即使身体化为灰烬。

"戏……不能散……" 他们唱。

你被无形的力量按在观众席上。大火蔓延到你身上，但你感觉不到疼。

你只是和他们一起唱，一遍又一遍，直到自己也变成戏台的一部分。`,
        ending: 'trappedAudience'
    },

    beg_for_exit: {
        text: `你跪下："求你们放我走！我什么都不知道！"

台下的观众齐刷刷地摇头。

"进了戏台，就是戏中人。" 他们说。

你被拖上台，戏服套在你身上，油彩画在你脸上。

这一次，你演的是一具尸体。一具在火中挣扎、却怎么也死不了的尸体。`,
        ending: 'trappedAudience'
    },

    confront_liu: {
        text: `"是你放的火。" 你说。

刘掌柜的鬼魂笑了："是又怎么样？那丫头抢了我女儿的风头，她该死。"

"可你烧死了整个戏班。"

"那又怎样？" 他的脸开始扭曲，"他们每晚都唱，唱得我不得安宁。你来正好，你来替我们听完！"

他扑向你。你举起云袖的银钗，钗尖刺入他的胸口。

刘掌柜发出一声惨叫，化成黑烟消散了。

但戏台上的锣鼓声没有停。

云袖的声音响起："火因灭了，但戏，还没散。"`,
        effects: {
            yin: -10,
            setFlag: 'liuDestroyed'
        },
        choices: [
            {
                text: '上台完成演出',
                next: 'perform_opera'
            }
        ]
    },

    retreat_slowly: {
        text: `你慢慢后退，想趁刘掌柜不注意溜走。

但你的脚踩到了一根木棍。刘掌柜猛地转头，眼中燃起鬼火。

"想跑？" 他狞笑着，"当年的火，还差一个人祭。"

你转身就跑，但戏台四周突然升起火墙。这不是真正的火，是三十年前的怨火，烧不死肉体，却能焚毁灵魂。

你被火墙吞没。`,
        ending: 'soulBurned'
    },

    accept_yunxiu: {
        text: `你放下所有防备，看着云袖的眼睛。

她的眼睛很美，像两潭深不见底的井水。

"我陪你唱。" 你说。

云袖笑了。她牵起你的手，和你一起唱完《牡丹亭》。

台下的观众开始鼓掌，掌声越来越响，越来越响。

曲终时，你发现自己已经不会离开了。你也不想了。

从此，青石戏台多了一个小生。每夜子时，他和杜丽娘同台演出。

没人知道他们唱了多少年。

只知道那出戏，永远不会散场。`,
        ending: 'eternalDuet'
    },

    finish_opera: {
        title: '曲终',
        text: `你闭上眼睛，用尽最后的力气唱完最后一句：

[whisper]"则为你如花美眷，似水流年，是答儿闲寻遍……"[/whisper]

云袖的动作停住了。

戏台上的红灯笼一盏接一盏熄灭。台下的观众渐渐淡去。

你感到戏服从身上脱落，脸上的油彩化为灰烬。

天亮了。

你瘫坐在戏台上，浑身湿透。戏台还是那个破败的戏台，仿佛什么都没发生过。

但你知道，你替他们唱完了。

你走出青石镇，再也没有回去。`,
        effects: {
            sanity: -10,
            yin: -20
        },
        ending: 'finalCurtain'
    },

    // 曲终人散前置（新增分支）
    bury_yunxiu_memory: {
        title: '立碑',
        text: `你在青石戏台的废墟边，用石头和木板搭起一块简易的碑。

碑上没有刻字，只有一支录音笔，里面录下了你刚才听到的唱腔。

"云袖，" 你对着戏台说，"你的名字，你的戏，我会带回去。以后每一个研究民俗的人，都会知道春香班，知道《牡丹亭》，知道你。"

戏台上的红灯笼一盏接一盏熄灭。

云袖的身影出现在戏台边缘。她没有五官的脸上，似乎浮现出一个淡淡的笑。

"谢谢。" 她说，"这出戏，终于可以散了。"

她的身影化作一阵风，吹过废墟，吹向远方。

天亮了。青石戏台再也没有唱过鬼戏。`,
        effects: {
            sanity: 15,
            yin: -30,
            setFlag: 'yunxiuRemembered'
        },
        choices: [
            {
                text: '带着录音笔离开',
                next: '',
                ending: 'hidden_songEnds'
            }
        ]
    },

    // 曲终人散前置：戏服安葬（新增分支）
    bury_yunxiu_costume: {
        title: '葬衣',
        text: `你在戏台废墟边挖了一个浅坑，把云袖的戏服和银钗一起放进去。

"你不想再唱一辈子。" 你说，"我帮你把这套戏服葬了。从今以后，你只是云袖，不是杜丽娘。"

戏服接触泥土的瞬间，你听见一声似有似无的叹息。

坑填平了。你在上面放了一块石头，用朱砂写下两个字：

[faded]"云袖"[/faded]

风过戏台，幕布轻轻飘动，像是在向你作揖谢幕。`,
        effects: {
            sanity: 10,
            yin: -25,
            setFlag: 'yunxiuCostumeBuried'
        },
        choices: [
            {
                text: '离开青石镇',
                next: '',
                ending: 'hidden_songEnds'
            }
        ]
    },

    // 隐藏结局分支：慈悲戏
    xiulan_wisdom: {
        text: `你没有上台。

你想起秀兰。那个被活埋在山村井底三十年的新娘，最后要的不过是一句道歉，一个名分。

云袖和秀兰一样，都是被辜负的女子。周生辜负了她，刘掌柜害死了她。戏台困住她，不是因为戏没唱完，是因为她心里还有恨。

你站起身，对着台上的云袖说：

"云袖，我知道你不是要等柳梦梅。你是要等一句对不起。"

台上的戏子们停下了动作。云袖缓缓转向你，没有五官的脸上似乎有什么在流动。

"周生就在后台。" 你说，"他不敢见你，但他用三十年在赎罪。你若有恨，恨他一人；你若要公道，我替你写进县志。"

你取出笔记本，在戏单的背面写下云袖的名字、春香班的名字、还有那场大火的真相。

戏台上的红灯笼一盏接一盏熄灭。云袖走到你面前，伸出透明的手，轻轻覆在你的笔记本上。

"够了。" 她说，"有人记得我，就够了。"

她的身影随着最后一句唱腔消散。戏台外，天亮了。`,
        effects: {
            sanity: 15,
            yin: -30,
            setFlag: 'yunxiuMercy'
        },
        ending: 'hidden_operaOfMercy'
    },

    // 隐藏结局分支：真言
    truth_for_yunxiu: {
        text: `你走到台前，云袖正在等。

"我知道真相了。" 你说，"放火的是刘掌柜，不是周生。周生想救你，但他没能做到。"

云袖的身体颤抖了一下。

"我也知道，" 你继续说，"你要的不是周生死，你要的是他承认。承认他害了你，承认他对不起你。"

你转向后台，对镜子里那个不敢出来的身影喊道："周生，出来。三十年的戏，该散了。"

镜子里传来压抑的哭声。周生的身影慢慢走出来，脸上终于有了五官——是一个清秀却憔悴的年轻人。

"云袖，" 他跪下来，"对不起。那天我不该嫉妒，不该在戏台底下倒桐油。我以为只要戏唱不成，你就会跟我走。我没想到……"

云袖走到他面前，伸出手，却穿过了他的身体。

"我知道。" 她说，"我早就知道。所以我才不怪你。"

[red]原来她等的，从不是复仇，是这句道歉。[/red]

戏台上的红灯笼全部熄灭。当晨光再次照进戏台时，你看见戏台废墟上，一对穿戏服的身影手牵着手，慢慢走远。`,
        effects: {
            sanity: 20,
            yin: -30,
            setFlag: 'yunxiuForgiven'
        },
        ending: 'hidden_trueWord'
    }
};

export const Endings = {
    finalCurtain: {
        title: '真结局 · 曲终人散',
        text: `你唱完了《牡丹亭》。

云袖和周生的执念终于消散，青石戏台恢复了平静。

你回到研究所，把这段经历写成报告。同事们都说你疯了，但你知道那是真的。

后来，青石镇重建了戏台。开光那天，唱的是《牡丹亭》。

你站在台下，听见一个女子的声音轻轻说："谢谢。"

你抬头，台上空无一人。只有风吹过幕布，像是有人在谢幕。`
    },
    hidden_songEnds: {
        title: '隐藏结局 · 曲终人散',
        text: `你没有陪云袖唱完那出戏。

你帮她立了碑，葬了戏服，把她的名字和唱腔录进录音笔。你让她明白：戏可以散，但人会记得。

回到研究所后，你把录音整理成资料，附在关于青石戏台的报告后面。几年后，一位戏曲学院的教授听了这段录音，激动得说不出话。

"这是春香班的唱腔。" 他说，"失传近百年的春香班。"

青石戏台重建时，你捐出了那块写着"云袖"的石头。它现在立在戏台入口，成了戏台的一部分。

每年中元节，戏台都不唱《牡丹亭》。

因为那一出戏，终于散了。`
    },
    reunion: {
        title: '真结局 · 团圆',
        text: `云袖知道了真相，周生也得到了救赎。

他们在镜中相会，一起离开了困住他们三十年的戏台。

你走出青石镇时，天边泛起鱼肚白。你回头看了最后一眼，戏台的废墟上，似乎有一对穿戏服的身影手牵着手，慢慢走远。

你打开录音笔，里面只有一段杂音。但仔细听，能听出是两个声音在合唱。

一男，一女。

曲终，人散，缘尽。`
    },
    peacefulDeparture: {
        title: '普通结局 · 归还',
        text: `你还回了云袖的银钗。

她没有为难你，只是唱了一段戏，然后离开了。

你平安回到研究所，但那个唱腔一直萦绕在你耳边。

有时候半夜醒来，你会听见窗外有人在唱《牡丹亭》。

你不敢看窗外。你知道，她可能还在找下一个愿意听她唱完的人。`
    },
    eternalDuet: {
        title: '死亡结局 · 永世同台',
        text: `你接受了云袖，成了她的柳梦梅。

青石戏台每晚都会开演《牡丹亭》，台上的小生和旦角永远年轻，永远深情。

偶尔有路人经过，会听见戏台上传来锣鼓声和唱腔。他们以为是镇上在唱戏。

只有你知道，那出戏已经唱了三十年，还会再唱三百年。

因为你，永远不会让戏散场。`
    },
    trappedAudience: {
        title: '死亡结局 · 座中客',
        text: `你被困在戏台下，成了永远的观众。

云袖和周生唱了一遍又一遍，每次唱的都不一样，但每次都唱不到结尾。

你坐在台下，不能动，不能走，不能闭眼。

你的耳朵永远充斥着昆曲，你的眼睛永远看见红色的戏服。

直到有一天，你听见身后有人坐下。

又是一个新来的研究员。

你转过头，想提醒他快逃。但你的嘴张不开。

你只能看着他，一步步走进戏台，然后变成下一个座中客。`
    },
    becomeYunxiu: {
        title: '死亡结局 · 借身',
        text: `你带走了云袖的银钗，云袖便借走了你的身体。

她穿着你的衣服，用你的名字，回到了城里。

而你，被困在烧焦的戏台上，穿着她的红戏服，唱着她的《牡丹亭》。

每当有人举起相机，你都会转向镜头。

照片里，你的脸一片空白。`
    },
    lostInFog: {
        title: '死亡结局 · 雾中迷',
        text: `你在雾中迷失了方向。

你跑了一辈子，其实只是在戏台周围打转。

当你终于跑不动时，你发现自己站在戏台上。台下坐满了没有脸的人，他们等着你开唱。

你张开口，发现自己只会唱《牡丹亭》了。

原来，你早就成了戏中人，只是自己不知道。`
    },
    becomeCostume: {
        title: '死亡结局 · 戏衣',
        text: `你和戏服长在了一起。

你的身体变成了红色的绸缎，你的脸成了戏服上的脸谱。

云袖把你挂在后台，和其他戏服挂在一起。

每当有新的"柳梦梅"上台，她就会把你取下来，穿在他身上。

你成了戏台的一部分，永远。`
    },
    soulBurned: {
        title: '死亡结局 · 焚魂',
        text: `怨火吞没了你的灵魂。

你的身体还站在戏台边，但里面已经空了。

云袖用你的身体，完成了最后一场演出。

台下观众鼓掌时，你的身体也在鼓掌。

但你知道，那不是你。

你只是一团灰烬，散落在青石戏台的废墟里。`
    },

    hidden_operaOfMercy: {
        title: '隐藏结局 · 慈悲戏',
        text: `你没有唱完《牡丹亭》。

你用一本笔记本，替云袖写下了她想要的公道。她的名字、春香班的名字、还有那场大火的真相，都被你带回了研究所。

三年后，青石镇戏台重建。开光那天，唱的是《牡丹亭》。但你特意在戏台边立了一块碑，刻着十七个名字。

云袖的名字，排在第一个。

从那以后，青石镇再也没有人听过鬼戏。偶尔有风吹过戏台，人们会说是云袖在谢幕。

你成了镇上最不受欢迎的研究员，因为你揭开了他们不愿面对的真相。

但你不在乎。

[red]有些戏，本来就不该唱完。[/red]`
    },

    hidden_trueWord: {
        title: '隐藏结局 · 真言',
        text: `你让周生亲口说出了"对不起"。

三十年的执念，就因为这一句真言，消散了。云袖和周生手牵着手离开，戏台终于安静下来。

你走出青石镇时，天边泛起鱼肚白。你打开录音笔，里面只有一段杂音。

但仔细听，能听出是两个声音在说话。

"对不起。"

"我知道。"

仅此而已。

[red]有时候，世界上最厉害的法器，不是铜镜，不是黄符，而是一句真心的道歉。[/red]`
    },

    hidden_photoAshes: {
        title: '隐藏结局 · 焚照',
        text: `你把那张烧焦的胶卷烧了。

火苗舔上胶卷的瞬间，你听见身后传来一声轻轻的叹息。回头一看，戏台上站着十几个穿戏服的人，他们朝你鞠了一躬，然后一个个消失在火焰里。

"谢谢你。"云袖说，"我们被相机困了很久。"

你明白，那些没有五官的相片，其实是被困住的魂魄。你把它们烧掉，它们就自由了。

录音笔里只留下一阵风声，像是一群人在远处鼓掌。

戏台终于空了。`
    },

    hidden_stageWitness: {
        title: '隐藏结局 · 见证',
        text: `你没有登台，也没有离开。

你坐在台下，看完了整场《牡丹亭》。云袖唱得很好，周生也唱得很好。他们终于把戏唱完了。

曲终时，云袖看着你，说："谢谢你愿意看完。"

"这是我应该做的。"你说，"观众不散，戏不散。"

她笑了笑，和周生一起走下戏台。他们的身影在晨光中渐渐透明，最后消失不见。

你起身离开青石镇。 your report (你的报告)里多了一段详细的观剧记录。

同事们说你疯了，但你不在乎。因为你亲眼看见，一出唱了三十年年的鬼戏，终于落幕。`
    },

    hidden_liuConfession: {
        title: '隐藏结局 · 刘掌柜的供词',
        text: `你找到了刘掌柜的账本。

账本里记载着他购买桐油的日期、数量和用途。你把账本交给了青石镇的派出所。

虽然刘掌柜已经死了，但真相终于大白。青石镇的老人们在戏台废墟前烧掉了账本，算是给春香班一个交代。

云袖站在人群外，静静地看着。

"不是对不起。"她说，"是公道。"

你点点头："对，是公道。"

那天夜里，戏台的锣鼓声响了最后一次，然后彻底安静下来。

青石镇的人说，那是春香班在谢幕。`
    },

    hidden_zhouDescendant: {
        title: '隐藏结局 · 周生后人',
        text: `你找到了周生的后人，一个年轻的戏曲演员。

你请他来青石镇，在戏台废墟前唱一段《牡丹亭》。他起初不愿意，但听完故事后，答应了。

他唱的是"原来姹紫嫣红开遍"，唱得很好。唱到一半，戏台上出现了一个穿红戏服的身影。

云袖站在他面前，泪流满面。

"你爷爷没唱完的，"年轻人说，"我替他唱完。"

云袖点点头，和他一起唱完了最后一句。

曲终，她朝周生后人福了一福，化作红光散去。

周生后人跪在地上，久久不能起身。`
    },

    hidden_yunxiuRequiem: {
        title: '隐藏结局 · 安魂曲',
        text: `你没有唱《牡丹亭》，而是给云袖唱了一首安魂曲。

那曲子没有词，只是简单的哼唱。你的声音在空旷的戏台上回荡，像是一阵风穿过废墟。

云袖听着听着，慢慢平静下来。她脸上的面具碎了，露出原本清秀的容颜。

"我不唱《牡丹亭》了。"她说，"我想休息了。"

你继续哼唱，直到她的身影完全消散。

戏台上的红灯笼一盏盏熄灭。你把银钗留在台上，转身离开。

青石镇从此再也没有听过鬼戏。`
    },

    hidden_noFaceAudience: {
        title: '隐藏结局 · 无面观众',
        text: `你决定不登台，也不做观众。

你走到那些没有脸的"观众"面前，对他们说："散了吧。戏已经唱完了，你们不用再等。"

他们一动不动。

你又说："云袖不是你们的戏。你们也不是她的观众。你们只是困在这里的魂。"

话音刚落，他们的脸开始恢复五官。有老人，有孩子，有男人和女人。他们都是当年死在戏台下的村民。

他们朝你点点头，然后一个个站起来，向戏台外走去。

云袖站在台上，看着他们离开，轻声说："原来，他们也在等我散场。"

戏台空了。`
    },

    hidden_redLantern: {
        title: '隐藏结局 · 红灯笼',
        text: `你取下了戏台上最后一盏红灯笼。

灯笼里不是蜡烛，而是一团幽幽的鬼火。你把灯笼带到河边，放到水面上，看着它漂远。

"云袖，"你说，"跟着它走，去该去的地方。"

灯笼在水面转了三圈，然后顺流而下。你听见远处传来一阵唱戏声，渐渐消失。

第二天，青石镇的人发现戏台上的红灯笼全都不见了。有人说昨晚看见一盏红灯漂在河上，以为是河灯。

你知道，那盏灯带着云袖，去了彼岸。`
    },

    hidden_openerCurtain: {
        title: '隐藏结局 · 启幕人',
        text: `你没有让戏散场，而是让戏重新开始。

你在青石戏台的废墟上搭了一座新戏台，请来了真正的戏曲演员，演了一出完整的《牡丹亭》。

开演那天，台下坐满了人。云袖坐在最后一排，穿着普通人的衣服，看着台上的演员唱她唱过的词。

"他们唱得比我好。"她说。

"但他们没有你等了那么久。"你说。

她笑了。

戏演完后，她把银钗交给你："替我保管。等下一出戏开演时，再把它放回去。"

你答应了。`
    },

    hidden_eternalStage: {
        title: '隐藏结局 · 永恒戏台',
        text: `你没有破坏戏台，也没有超度云袖。

你在戏台边搭了一座小木屋，住了下来。每天你打扫戏台，给云袖上香，偶尔给她唱一段《牡丹亭》。

游客们以为你是守庙人，会给你一些香火钱。你用这些钱修缮戏台，还在旁边立了一块碑，刻着春香班十七个人的名字。

云袖不再闹鬼。她成了青石镇的一个传说，一个守护戏台的精灵。

有时候月圆之夜，戏台上会出现一个红色的身影，唱一两句就消失。

游客们以为是表演效果。只有你知道，那是云袖在谢幕。`
    }
};

// 低理智幻觉
StoryData.prologue.hallucination = "[whisper]你低头看自己的影子，发现影子的姿势和你不一样——它在作揖。[/whisper]";
StoryData.return_to_stage.hallucination = "[whisper]你仿佛听见无数戏子在耳边齐声说：'来了，就不要走了。'[/whisper]";
StoryData.perform_opera.hallucination = "[whisper]你看见台下的观众都有了脸，每一张脸都是你自己。[/whisper]";

export default { StoryData, Endings };


// ==================== 新增场景与结局 ====================

// 新增场景：河边
StoryData.river_bank = {
    title: '河边',
    text: `你顺着纸条上的水渍，找到了青石镇外的老河。

河水比记忆中黑得多，像是一坛打翻的墨。岸边生着半枯的芦苇，风一过，苇秆互相敲击，发出类似板鼓的声响。

你蹲在河边，听见水底有人在唱：

[whisper]"来呀，水凉快呀……
水下没有孤灯，水下没有长夜……"[/whisper]

那声音很细，像是从很远的地方传来，又像就在你耳边。`,
    effects: {
        sanity: -10,
        yin: 15
    },
    choices: [
        {
            text: '下水寻找声音来源',
            next: 'dive_river'
        },
        {
            text: '在岸边接着唱',
            next: 'sing_at_river'
        },
        {
            text: '把纸条投入河中',
            next: 'return_to_stage',
            effects: { sanity: 5, yin: -5 }
        }
    ]
};

StoryData.dive_river = {
    title: '水下',
    text: `你踏入河水。

水比想象中暖，像是谁的体温。你往下沉，看见河底沉着许多东西：戏服、头面、断了弦的琴。

最深处，站着一个穿绿衣的女子。她的脸被水泡得发白，手里攥着一张戏单。

你认出那是春香班的备选旦角，刘掌柜的女儿。她当年没有烧成灰，而是投河自尽，要陪着云袖一起唱。

她伸出手，把冰凉的手指扣在你腕上：

[whisper]"你来替我，我就能上岸了。"[/whisper]`,
    effects: {
        sanity: -30,
        yin: 30
    },
    ending: 'death_waterGhost'
};

StoryData.sing_at_river = {
    title: '河上和',
    text: `你没有下水，而是坐在岸边，清了清嗓子。

你唱的是《牡丹亭》里杜丽娘的念白，不是那段水词。你的声音在河面上荡开，水底的唱腔停了停，然后接了下去。

一人在岸上唱，一人在水下和。两段声音在河心相遇，像是两根断了的弦被重新接上。

唱到"则为你如花美眷"时，水面上浮起一张湿软的戏单。戏单上，刘掌柜女儿的名字旁，被人用朱砂补上了"云袖"二字。

[whisper]"原来我不是一个人。"[/whisper]

水底的女子轻轻叹了口气，河水忽然变得清澈。`,
    effects: {
        sanity: 5,
        yin: -20,
        setFlag: 'riverSungWithGhost'
    },
    ending: 'hidden_riverEcho'
};

StoryData.under_stage = {
    title: '台下',
    text: `你趴下来，从戏台侧面的缝隙钻入台底。

台底比想象中宽敞，四根台柱像四根骨头撑起一片黑暗。地上散落着油渍、炭灰，还有几枚生锈的铁钉。

你摸到台基中央有一块松动的石板。石板缝里透出微弱的红光，伴随着低沉的锣鼓声——原来戏台的声音，是从地底传上来的。`,
    effects: {
        sanity: -15,
        yin: 20
    },
    choices: [
        {
            text: '挪开石板查看',
            next: 'open_compartment'
        },
        {
            text: '躲在这里等天亮',
            next: 'hide_under_stage'
        },
        {
            text: '用银钗压住石板缝隙',
            next: 'return_to_stage',
            condition: { hasItem: '云袖的银钗' },
            hidden: true,
            effects: { yin: -10, sanity: 5 }
        }
    ]
};

StoryData.open_compartment = {
    title: '暗格',
    text: `你挪开石板，下面是一个小小的暗格。

暗格里放着一卷绣着龙纹的黄色幕布，幕布边缘烧焦，却被血染过一般红。你认出这是旧戏班开演前必挂的"龙王幕"，据说能镇水火。

幕布下压着一张纸条：

[faded]"春香班班主敬献龙王幕，佑我等水火不侵。民国二十三年七月十五。"[/faded]

原来当年班主曾请过龙王幕，但演出前夜，幕布被人偷走。没有龙王幕镇台，那场火才能烧得起来。

你展开幕布，龙纹在黑暗中微微发亮。`,
    effects: {
        sanity: 5,
        yin: -15,
        addItem: '龙王幕'
    },
    choices: [
        {
            text: '把龙王幕挂回戏台',
            next: 'hang_dragon_curtain'
        }
    ]
};

StoryData.hang_dragon_curtain = {
    title: '挂幕',
    text: `你爬回戏台，把龙王幕挂上横梁。

黄幕垂下的瞬间，戏台上的红灯笼齐齐暗了一瞬，然后重新亮起——这一次，光是暖的。

云袖的身影出现在幕前。她看着那卷龙纹，缓缓跪下，像一个真正的旦角在谢幕。

"班主说过，龙王幕在，戏班就在。"她的声音很轻，"谢谢你们，把它送回来了。"

锣鼓声渐渐低了下去，像是有人轻轻盖上了琴盒。

天快亮了。`,
    effects: {
        sanity: 15,
        yin: -30,
        setFlag: 'dragonCurtainHung'
    },
    ending: 'hidden_dragonKingCurtain'
};

StoryData.hide_under_stage = {
    title: '台下囚',
    text: `你缩在台底，抱着膝盖，等待天亮。

但天一直没有亮。锣鼓声越来越近，像是有人在你头顶上来回走动。你抬头，看见木板缝隙间垂下许多戏服的水袖，红红白白，像一片倒生的水草。

"台下还差一个垫场的。"云袖的声音从台上传来，"你来垫，戏就稳了。"

你想起老人们说的话：戏台底下要埋一个"台基魂"，戏台才不会塌。当年春香班没有埋，所以他们遭了火。现在，云袖要你做那个台基魂。

你张嘴想喊，却发现嘴里塞满了泥土和纸灰。`,
    effects: {
        sanity: -40,
        yin: 30
    },
    ending: 'death_foundation'
};

StoryData.paper_shop = {
    title: '周氏纸扎铺',
    text: `你按照老头说的方向，找到了青石镇东头的纸扎铺。

铺子早塌了半边，但柜台后的架子上还摆着许多纸扎：纸人、纸马、纸轿，糊得栩栩如生。最显眼的是一个穿红戏服的纸女，眉眼和云袖有七分像。

柜台上有本残破的账本，最后一页写着：

[faded]"周生订：纸女一具，红衣水袖，七月十五取。"[/faded]

原来周生在火灾前夜，曾来这里订过一具纸人。他想用纸人代替云袖演完那场戏，好带她私奔。

但纸人没来得及取，火就烧了。`,
    effects: {
        sanity: -10,
        yin: 15,
        setFlag: 'knowsPaperDoll'
    },
    choices: [
        {
            text: '为纸女点睛',
            next: 'paper_substitute'
        },
        {
            text: '烧掉纸女',
            next: 'burn_paper_doll'
        }
    ]
};

StoryData.paper_substitute = {
    title: '纸人替',
    text: `你取出一支毛笔，蘸了柜台上的朱砂，为纸女点上眼睛。

纸女的眼睛亮了起来。她缓缓从架子上走下来，水袖一甩，竟真有几分云袖的神韵。

"我替她唱。"纸女说，声音像是从很远的地方传来，"她等了太久，该歇一歇了。"

你带着纸女回到戏台。云袖站在台上，看着这个和自己七分像的纸人，愣了很久。

"原来他做了这个。"她轻声说，"他原来，真的想带我走。"

纸女登台，云袖退后。一出《牡丹亭》，终于有两个"杜丽娘"同时出现——一个唱给人听，一个唱给鬼听。`,
    effects: {
        sanity: -5,
        yin: -20
    },
    ending: 'hidden_paperPuppet'
};

StoryData.burn_paper_doll = {
    title: '焚偶',
    text: `你点燃纸女。

火苗蹿起的瞬间，纸女发出一声凄厉的尖叫。那不是纸燃烧的声音，是云袖的声音。

[whisper]"你烧了我和他唯一的念想……"[/whisper]

灰烬里浮现出一只手，抓住了你的手腕。纸女的五官在火焰中扭曲，最后定格成你的脸。

"你来替她。"她说。`,
    effects: {
        sanity: -30,
        yin: 25
    },
    ending: 'trappedAudience'
};

StoryData.willow_tree = {
    title: '老柳树',
    text: `周生的后人把你带到镇外一棵半枯的老柳树下。

"我爷爷死后，家里人在这树下挖出过一支柳笛。"老人说，"他年轻时给云袖做的，云袖最爱听他吹。"

你在树下挖了挖，果然摸到一支柳笛。笛身已经发黑，但孔洞还通着气。笛尾刻着两个小字：

[faded]"袖生"[/faded]

你把柳笛举到唇边，试着吹了一声。笛音很低，像是一声叹息。`,
    effects: {
        sanity: -5,
        yin: 10,
        addItem: '柳笛'
    },
    choices: [
        {
            text: '把柳笛带上戏台',
            next: 'play_willow_flute'
        },
        {
            text: '把柳笛埋回去',
            next: 'return_to_stage',
            effects: { sanity: 5, yin: -5 }
        }
    ]
};

StoryData.play_willow_flute = {
    title: '柳笛声',
    text: `你带着柳笛回到戏台。

云袖正在台上独自唱《惊梦》，唱到"原来姹紫嫣红开遍"时，你把柳笛放到唇边。

笛声一起，云袖的唱腔就停了。

她缓缓转过头，没有五官的脸上，似乎有什么在流动。她向你走来，不是飘，是走，一步一步，像当年从后台走向台前。

"他吹过的。"她说，"他答应过我，唱完这出，就吹这支笛子给我听。"

你继续吹，声音断断续续。云袖站在你面前，红绳从腰间滑落。`,
    effects: {
        sanity: 10,
        yin: -25,
        setFlag: 'willowFlutePlayed'
    },
    ending: 'hidden_willowFlute'
};

StoryData.instrument_room = {
    title: '乐器房',
    text: `你绕到后台角落，发现一间塌了半边的乐器房。

房里堆满了破损的乐器：裂了皮的鼓、断了弦的琴、变了形的唢呐。最中间摆着一把二胡，琴筒上刻着"春香班"三个字。

你拿起二胡，发现它的弦不是普通的丝弦，而是两根细细的长发。头发缠在琴轴上，已经发黑。

这应该是云袖的头发。戏班的人用她的头发做弦，想让她的唱腔永远留在琴里。`,
    effects: {
        sanity: -15,
        yin: 20
    },
    choices: [
        {
            text: '扯断琴弦',
            next: 'break_strings'
        },
        {
            text: '拉一曲《惊梦》',
            next: 'perform_opera',
            effects: { sanity: -5, yin: 5 }
        }
    ]
};

StoryData.break_strings = {
    title: '断弦',
    text: `你抓住琴弦，用力一扯。

"铮"的一声，琴弦断了。不是普通的断裂声，像是一个女子的尖叫戛然而止。

二胡琴筒里飘出一缕青烟，在空中凝成云袖的脸。她看着你，没有愤怒，只有一种解脱。

"终于不用唱了。"她说。

你继续扯断其他乐器上残留的弦、敲裂鼓面。每毁掉一件乐器，戏台上的锣鼓声就弱一分。

最后，整个戏台安静下来。`,
    effects: {
        sanity: -10,
        yin: -20,
        setFlag: 'instrumentsDestroyed'
    },
    ending: 'hidden_brokenString'
};

// 新增结局
Endings.death_waterGhost = {
    title: '死亡结局 · 水下替',
    text: `你成了她的替身。

刘掌柜的女儿踩着你的肩膀浮上水面，而你留在河底，穿着那件湿漉漉的绿戏服。

每到雨夜，青石镇的河边就会传来唱戏声。老人们说，那是云袖在河里找的新的伴儿。

你的报告永远没写完。

你的相机在河岸上被人捡走，里面的照片洗出来，每一张都多了一个穿绿衣的模糊身影，站在你身后。`
};

Endings.hidden_riverEcho = {
    title: '隐藏结局 · 河上和',
    text: `你没有驱散河里的鬼魂，你只是陪她唱了一段。

回到研究所后，你把这段经历写进报告。同事都觉得你疯了，但你的录音笔里确实录到了两段唱腔，一粗一细，一高一低，配合得天衣无缝。

几年后，青石镇治理河道，从河底捞出一具穿着绿戏服的骸骨。她手里攥着一张戏单，背面写着：

[faded]"云袖姐，我终于不是一个人唱了。"[/faded]

你在戏单复印件前站了很久。

有些戏，不是非要散场。有人和声，就不算孤唱。`
};

Endings.hidden_dragonKingCurtain = {
    title: '隐藏结局 · 龙王幕',
    text: `你把龙王幕重新挂回了青石戏台。

那卷黄幕在风中轻轻飘动，龙纹上的金线在晨光里一闪一闪。戏台上再也没有出现过无脸的戏子，但每年七月半，镇上的人会听见一阵很轻的锣鼓声，响三下就停。

老人们说，那是春香班在拜龙王。

你在研究报告里附上了龙王幕的照片。有专家认出那是民国时期的戏台镇物，价值不菲。但你没有把它带走。

有些东西，就该留在它该留的地方。`
};

Endings.death_foundation = {
    title: '死亡结局 · 台下土',
    text: `你被埋进了戏台底下。

不是真正的土，而是戏班几十年来积攒的灰烬、碎纸、断弦和烧焦的戏服。它们一层一层覆在你身上，把你砌成台基的一部分。

从此，青石戏台站得更稳了。

每当有人登台，你都能感觉到震动。你成了戏台的骨头，戏台的根。

云袖唱戏时，你会跟着锣鼓的节奏，在土里轻轻摇晃。

你永远不会散场。`
};

Endings.hidden_paperPuppet = {
    title: '隐藏结局 · 纸人替',
    text: `你让纸人替云袖唱完了那出戏。

纸女在台上唱，云袖在台下看。曲终时，纸女化作一堆灰烬，被风吹散在戏台上。云袖伸手去接，却只接到一把细灰。

"他欠我的，还了。"她说。

她转身走向晨光，红戏服一点点变得透明。在彻底消失前，她回头看了你一眼：

[whisper]"告诉周生，我不怪他。从来都没怪过。"[/whisper]

青石戏台从此安静下来。你把那具纸女的灰烬收进一个小盒，和研究资料放在一起。

有些替身，比真人更真心。`
};

Endings.hidden_willowFlute = {
    title: '隐藏结局 · 柳笛声',
    text: `你吹响了周生留给云袖的柳笛。

笛声不响，却在戏台上绕了很久。云袖听完，朝你福了一福，像谢幕的旦角。

"我听过这笛子了。"她说，"可以走了。"

她走向晨光，红戏服化作漫天柳絮。你握着那支发黑的柳笛，站在空荡荡的戏台上。

回到城里，你把柳笛捐给了戏曲博物馆。展览牌上写着："民国二十三年，青石镇春香班遗物。"

每到春天，展柜附近总会飘过几片柳絮。游客们以为是空调风，只有你知道，那是云袖在听笛。`
};

Endings.hidden_brokenString = {
    title: '隐藏结局 · 断弦清',
    text: `你毁掉了春香班遗留的乐器。

云袖的唱腔不再被琴弦囚禁，周生的魂魄也不再被困在锣鼓里。戏台上最后一个红灯笼熄灭时，你听见许多人同时叹了一口气。

那不是悲伤，是解脱。

你把断弦和碎琴收进纸箱，带回研究所。同事们问你这是什么，你说："一组民国戏班的遗物，已经不会再响了。"

青石戏台重建那天，你把断弦埋在戏台底下，当作给春香班的祭品。

有些戏，不用唱完。把弦断了，大家就都能走了。`
};

// 为已有场景追加新选择分支
StoryData.backstage.choices.push(
    { text: '去河边寻找这段唱词的源头', next: 'river_bank' },
    { text: '检查后台角落的乐器房', next: 'instrument_room' }
);

StoryData.return_to_stage.choices.push(
    { text: '钻到戏台底下查看', next: 'under_stage' }
);

StoryData.ask_villagers.choices.push(
    { text: '去周氏纸扎铺旧址看看', next: 'paper_shop' }
);

StoryData.zhou_descendants.choices.push(
    { text: '他有没有留下什么遗物？', next: 'willow_tree' }
);

// 为已有场景追加 NPC 对话入口
StoryData.backstage.choices.push(
    { text: '和角落里敲鼓的老人说话', npc: 'lao_zhao', npcNode: 'start' }
);

StoryData.paper_shop.choices.push(
    { text: '和柜台后的周婆搭话', npc: 'zhou_po', npcNode: 'start' }
);

StoryData.river_bank.choices.push(
    { text: '试着和水中女子搭话', npc: 'liu_niang', npcNode: 'start' }
);

export const NPCs = {
    lao_zhao: {
        name: '老赵',
        title: '春香班司鼓',
        dialogue: {
            start: {
                text: '后台的阴影里坐着一个干瘦老人，膝盖上横着一对枣木鼓槌。他听见脚步声，抬起眼皮。\n\n"三十年没人敢进这后台了。你是来听戏的，还是来散戏的？"',
                choices: [
                    { text: '打听当年的事', next: 'ask_past' },
                    { text: '你能帮我什么？', next: 'ask_help', condition: { flag: 'knowsArson' } },
                    { text: '离开', exit: true }
                ]
            },
            ask_past: {
                text: '老赵用鼓槌敲了敲膝盖，发出沉闷的咚咚声。\n\n"民国二十三年七月十五，春香班唱《牡丹亭》。我敲到第三通开场鼓，就闻见台下有桐油味。火是从台板缝里蹿上来的，像有人在地上浇了一圈。"',
                choices: [
                    { text: '谁放的火？', next: 'ask_arsonist' },
                    { text: '云袖当时怎样？', next: 'ask_yunxiu_past' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_arsonist: {
                text: '老赵摇摇头。\n\n"我没看见脸。只看见一双绸缎鞋，鞋底沾着桐油，在台板下头踱步。火起之前，那人站了整整一出《游园》的时间，像是在等戏唱到最热闹的时候才下手。"\n\n他顿了顿，压低声音："但我听见他念叨了一句——云袖不死，我女儿永无出头之日。"',
                effects: { npcAffinity: 5, setFlag: 'zhaoHeardArsonist' },
                choices: [
                    { text: '继续问云袖', next: 'ask_yunxiu_past' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_yunxiu_past: {
                text: '"云袖那孩子，是最后一个还在唱的。"老赵的声音低了下去，"戏服都烧着了，她还站在台口唱"原来姹紫嫣红开遍"。班主去拉她，被她甩开了。"\n\n"她说：戏开了，就要唱完。唱不完，魂就散不了。"',
                choices: [
                    { text: '她为什么非要唱完？', next: 'ask_why_sing' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_why_sing: {
                text: '老赵叹了口气。\n\n"戏班的规矩。开锣的戏，锣鼓不停，戏就不能散。云袖是角儿，她比谁都信这个。所以她把魂也唱进了戏里，等着有人替她演完柳梦梅。"\n\n"你要是有胆子登台，记住——唱到"则为你如花美眷"时，别看她的眼睛。"',
                choices: [
                    { text: '回去', next: 'start' }
                ]
            },
            ask_help: {
                text: '老赵端详你很久，把鼓槌递过来。\n\n"你既然知道是刘掌柜放的火，就该知道这场戏不是唱给人听的。拿着这对鼓槌，要是台上锣鼓乱了，就敲三下。三下，不能多，不能少。多了，戏子以为你在催场；少了，他们以为你不想听。"',
                effects: { npcAffinity: 10, addItem: '老赵的鼓槌' },
                choices: [
                    { text: '收下鼓槌', next: 'start' }
                ]
            }
        }
    },

    zhou_po: {
        name: '周婆',
        title: '纸扎铺守夜人',
        dialogue: {
            start: {
                text: '柜台后走出一个佝偻的老妇人，手里攥着一把还没糊完的竹篾。她看了你一眼，又看了看架子上的纸女。\n\n"别碰她。那是周生三十年前订的，他说要做一个和云袖一模一样的纸人。"',
                choices: [
                    { text: '你认识周生？', next: 'ask_zhou' },
                    { text: '纸人有什么用？', next: 'ask_paper' },
                    { text: '离开', exit: true }
                ]
            },
            ask_zhou: {
                text: '"怎么不认识？那孩子小时候常来我铺子里玩。火前一晚，他红着眼睛来订这具纸女，说要红衣水袖，眉眼要和云袖一个模子。"\n\n周婆用竹篾敲了敲柜台："他想用纸人替云袖唱完那场《牡丹亭》，好带云袖私奔。可纸女还没糊完，火就烧起来了。"',
                choices: [
                    { text: '他成功了吗？', next: 'ask_fail' },
                    { text: '云袖知道吗？', next: 'ask_yunxiu_knows' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_fail: {
                text: '"成功？"周婆苦笑，"纸女现在还挂在那儿呢。周生死了，云袖也成了鬼，这纸女没人点睛，就只是一堆竹篾和纸。"\n\n"不过，"她忽然压低声音，"要是有人用朱砂笔替她点上眼睛，她就能替云袖把那场戏唱完。云袖歇了，戏也就散了。"',
                choices: [
                    { text: '怎么点睛？', next: 'ask_how_eye' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_yunxiu_knows: {
                text: '"她不知道。周生想给她惊喜，想让她在唱完《惊梦》之后，看见台下有人捧着她的人偶等她。"\n\n周婆摇摇头："可有时候，惊喜等不到天亮。云袖到死都以为周生负了她，以为是他划掉了戏单上的名字。"',
                choices: [
                    { text: '回去', next: 'start' }
                ]
            },
            ask_paper: {
                text: '"纸人替真人，是老手艺。点上眼，纸女就有魂；穿上戏服，就能登台。但替角儿唱戏，唱的是替身的命。"\n\n"你若真想让云袖散场，就得让纸女替她唱完那出《牡丹亭》。纸女烧成灰，云袖的执念也就跟着散了。"',
                choices: [
                    { text: '给我朱砂笔', next: 'give_cinnabar' },
                    { text: '我再想想', next: 'start' }
                ]
            },
            ask_how_eye: {
                text: '"用朱砂笔，点双目。点了眼，纸女就是魂的容器，你要负责把她送上戏台。要是不送，她会反过来缠上点睛的人。"\n\n周婆从柜台底下摸出一支发黑的毛笔，笔杆上缠着褪色的红绳。',
                choices: [
                    { text: '接过朱砂笔', next: 'give_cinnabar' },
                    { text: '不要', next: 'start' }
                ]
            },
            give_cinnabar: {
                text: '"拿去吧。这是我守铺子三十年，剩的最后一点朱砂。但愿你用得上，也但愿你用不上。"',
                effects: { npcAffinity: 10, addItem: '朱砂笔' },
                choices: [
                    { text: '回去', next: 'start' }
                ]
            }
        }
    },

    liu_niang: {
        name: '柳娘',
        title: '河下绿衣',
        dialogue: {
            start: {
                text: '水面泛起一圈涟漪，一个穿绿戏服的女子从河底浮起半截身子。她的脸被水泡得发白，手里攥着一张湿软的戏单。\n\n"来呀，水凉快呀……"',
                choices: [
                    { text: '你是谁？', next: 'ask_identity' },
                    { text: '你父亲是刘掌柜？', next: 'ask_father', condition: { flag: 'knowsSuspects' } },
                    { text: '我陪你唱一段', next: 'sing_together' },
                    { text: '离开', exit: true }
                ]
            },
            ask_identity: {
                text: '"我是春香班的备选旦角。"她举起手里的戏单，"本来那场《牡丹亭》，该我唱杜丽娘的。可班主说，云袖姐的嗓子比我好，让我唱花神。"\n\n她的声音越来越轻："我爹不甘心。他说，云袖不死，我就出不了头。"',
                choices: [
                    { text: '你恨云袖吗？', next: 'ask_hate' },
                    { text: '你恨你爹吗？', next: 'ask_hate_father' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_hate: {
                text: '"恨过。"柳娘低下头，"可水底下太冷，我一个人唱了三十年的水词。现在我只想有人和我和一曲。"\n\n"云袖姐教我甩水袖，教我唱"袅晴丝"。火起的时候，她把我推出了台口。她不知道，推我出去之后，我爹的事就让我活不下去了。"',
                effects: { npcAffinity: 10 },
                choices: [
                    { text: '我和你唱', next: 'sing_together' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_hate_father: {
                text: '"他是我爹。"柳娘的声音像是从水底传来，"可他也烧了我上台的路。我投河那天，穿的是杜丽娘的戏服。我想，既然我爹说云袖不死我就唱不了杜丽娘，那我就穿着这身戏服，去水里唱给他听。"',
                choices: [
                    { text: '回去', next: 'start' }
                ]
            },
            ask_father: {
                text: '"别提他。"柳娘的身形晃了晃，"但你是来查真相的，是不是？我告诉你——火前那晚，我看见他在油灯下数桐油壶。他数了十七下，春香班正好十七个人。"\n\n"他还念叨：云袖不死，我女儿永无出头之日。"',
                effects: { npcAffinity: 5, setFlag: 'liuDaughterWitness' },
                choices: [
                    { text: '还有别的吗？', next: 'ask_father_more' },
                    { text: '回去', next: 'start' }
                ]
            },
            ask_father_more: {
                text: '"有。"柳娘抬起头，"我爹不知道，我宁可一辈子唱丫鬟，也不想云袖姐死。她是我师姐，是我唯一的朋友。"\n\n"你要是见到她，替我说一句——柳娘不是来抢戏的，柳娘是来和声的。"',
                effects: { npcAffinity: 10 },
                choices: [
                    { text: '回去', next: 'start' }
                ]
            },
            sing_together: {
                text: '你坐在岸边，清了清嗓子，唱起《牡丹亭》里杜丽娘的念白。\n\n柳娘接了下去。一人在岸上唱，一人在水下和，两段声音在河心相遇，像是两根断了的弦被重新接上。\n\n唱到"则为你如花美眷"时，她忽然停了，从怀里掏出一样东西递给你。',
                effects: { npcAffinity: 15 },
                next: 'give_jade'
            },
            give_jade: {
                text: '"这是我娘留给我的玉坠。你拿着，去戏台时交给云袖姐，就说——柳娘不唱了，柳娘想和她一起歇。"\n\n玉坠入手冰凉，上面刻着一个小小的"柳"字。',
                effects: { addItem: '柳娘的玉坠' },
                choices: [
                    { text: '我会转交', next: 'start' },
                    { text: '离开', exit: true }
                ]
            }
        }
    }
};
